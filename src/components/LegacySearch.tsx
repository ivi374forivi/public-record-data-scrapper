...
export function LegacySearch() {
  ...
  const handleSearch = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setResults(deduped)
      setStatus(`Found ${deduped.length} item${deduped.length === 1 ? "" : "s"} (duplicates filtered)`)
    } catch (err) {
          ...
          <Table className="min-w-full">
            ...
            <TableBody>
              {formattedResults.map((result, index) => (
                <TableRow key={`${result.url ?? result.title ?? index}-${index}`} className="border-white/5">
                  ...
                  <TableCell className="space-y-1">
                    ...
                    {result.snippet && (
                      <p className="text-sm text-white/70">{result.snippet}</p>
                    )}
                  </TableCell>
                  <TableCell>
                    {result.url ? (
